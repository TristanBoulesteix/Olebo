package jdr.exia.view.composable.master.temp
...
class MapPanel(private val isParentMaster: Boolean, private val viewModel: MasterViewModel) : JComponent() {
        ...
        addMouseReleasedListener { me ->
            ...
            selectedArea?.let {
                if (it.size >= Dimension(
                        Size.XS.size.absoluteSizeValue,
                        Size.XS.size.absoluteSizeValue
                    )
                )
    ...
    private fun relativeX(absoluteX: Int): Int {
        return (absoluteX * this.width) / MasterViewModel.ABSOLUTE_WIDTH
    }
    ...
    private fun relativeY(absoluteY: Int): Int {
        return (absoluteY * this.height) / MasterViewModel.ABSOLUTE_HEIGHT
    }
    ...
    private fun absoluteX(relativeX: Int): Int {
        return (((relativeX.toFloat() / this.width.toFloat())) * MasterViewModel.ABSOLUTE_WIDTH).toInt()
    }
    ...
    private fun absoluteY(relativeY: Int): Int {
        return (((relativeY.toFloat() / this.height.toFloat())) * MasterViewModel.ABSOLUTE_HEIGHT).toInt()
    }
